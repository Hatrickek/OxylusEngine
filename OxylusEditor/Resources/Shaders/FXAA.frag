...
layout (constant_id = 0) const float CurrentThreshold = 0.0833;
layout (constant_id = 1) const float RelativeThreshold = 0.125;
...
void main()
{
	...
	float dirReduce = max((lumaNW + lumaNE + lumaSW + lumaSE) * (0.25 * RelativeThreshold), CurrentThreshold);

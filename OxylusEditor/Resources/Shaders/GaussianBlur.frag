...
void main() {
  ...
  if (u_Const.IsHorizontal) {
    ...
    color = c0 * 0.01621622 + c1 * 0.05405405 + c2 * 0.12162162 +
    ...
    // equivalent)
    vec4 c0 =
        texture(u_Sampler, in_TexCoord - vec2(0.0, texelSizeY * 3.23076923));
    vec4 c1 =
        texture(u_Sampler, in_TexCoord - vec2(0.0, texelSizeY * 1.38461538));
    ...
    color = c0 * 0.07027027 + c1 * 0.31621622 + c2 * 0.22702703 +

...
public:
  ...
  template <typename T> static void register_sink(void* user_data) { external_sinks.emplace_back(std::make_shared<T>(user_data)); }
...
#define OX_INTERNAL_ASSERT_IMPL(type, check, msg, ...) \
  {                                                    \
    if (!(check)) {                                    \
      OX##type##ERROR(msg, __VA_ARGS__);               \
      OX_DEBUGBREAK();                                 \
    }                                                  \
  }
#define OX_INTERNAL_ASSERT_WITH_MSG(type, check, ...) OX_INTERNAL_ASSERT_IMPL(type, check, "Assertion failed: {0}", __VA_ARGS__)
#define OX_INTERNAL_ASSERT_NO_MSG(type, check)                       \
  OX_INTERNAL_ASSERT_IMPL(type,                                      \
                          check,                                     \
                          "Assertion '{0}' failed at {1}:{2}",       \
                          OX_STRINGIFY_MACRO(check),                 \
                          ::ox::FileSystem::get_file_name(__FILE__), \
                          __LINE__)

#define OX_INTERNAL_ASSERT_GET_MACRO_NAME(arg1, arg2, macro, ...) macro
#define OX_INTERNAL_ASSERT_GET_MACRO(...) \
  OX_EXPAND_MACRO(OX_INTERNAL_ASSERT_GET_MACRO_NAME(__VA_ARGS__, OX_INTERNAL_ASSERT_WITH_MSG, OX_INTERNAL_ASSERT_NO_MSG))
...
#ifndef OX_ENABLE_ASSERTS
  #undef OX_ASSERT
  #undef OX_CORE_ASSERT
  #define OX_ASSERT(...)
